...
public class FlatFileConnector implements PoolableConnector,CreateOp,UpdateOp,DeleteOp,SearchOp<Map<String,String>>,SchemaOp,TestOp {
...
public void executeQuery(ObjectClass objectClass, Map<String,String> query,
...
searchResult = getIncrementalSearchResult(reader, targetFile, uniqueAttribute,query);

}else{
System.out.println("Performing full recon");
searchResult = getSearchResult(reader, targetFile, uniqueAttribute,query);
}
...
private Map<String,HashMap<String,List<String>>> getIncrementalSearchResult(BufferedReader reader,File targetFile,String uniqueAttribute,Map<String,String> queryMap){
return getSearchResult(reader, targetFile, uniqueAttribute,queryMap);
}
private Map<String,HashMap<String,List<String>>> getSearchResult(BufferedReader reader,File targetFile,String uniqueAttribute,Map<String,String> queryMap){
...
// if yes we only need to return the result that matches the filter query
if(queryMap!=null){
searchAttribute=(String)queryMap.keySet().toArray()[0];
searchAttributeValue=queryMap.get(searchAttribute);
System.out.println(" **** Attribute "+searchAttribute +" Value "+searchAttributeValue);
}
...
System.out.println(" Attribute "+strSplit[0]+ " Value "+strSplit[1]);
...
// Check whether there is a search filter, if there is filter add only those rows that match the search query
if(searchAttribute!=null && searchAttributeValue !=null){
boolean valueFound=false;
ArrayList<String> attributeValues = (ArrayList<String>) values.get(searchAttribute);
for(String value:attributeValues){
System.out.println(" Searching through "+searchAttribute +" Current vaue is "+value);
if(value.startsWith(searchAttributeValue)){
valueFound=true;
System.out.println(" Value "+ value +" Starts with "+searchAttributeValue +" Need to add to Recon Result");
break;
}
}
if(valueFound){
...
}else if(searchAttribute ==null && searchAttributeValue ==null){
searchResult.put(uniqueAttrValue, values);
}
}
} catch (IOException e) {
...
attrInfoSet.add(AttributeInfoBuilder.build(FlatFileUtil.Password, String.class));
