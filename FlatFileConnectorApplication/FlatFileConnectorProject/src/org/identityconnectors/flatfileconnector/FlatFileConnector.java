...
public class FlatFileConnector implements PoolableConnector, CreateOp,
                                          UpdateOp, DeleteOp,
    ...
    private void createSshConnection() {
        ...
        this.config.getPassword().access(new GuardedString.Accessor() {
                ...
                public void access(char[] arg0) {
                    try {
                        session =
                                new SshSession(config.getHostName(), config.getUserName(),
                                               new String(arg0));
        ...
        System.out.println("Successfully connected to the target system " +
                           this.config.getHostName() + " with user name " +
    ...
    public Uid create(ObjectClass oclass, Set<Attribute> attrs,
        ...
        if (customScriptFile != null) {
            ...
            ProcessBuilder pb =
                new ProcessBuilder(customScriptFile.getAbsolutePath(), id,
    ...
    public void executeQuery(ObjectClass objectClass,
        ...
        if (reconType.equalsIgnoreCase(FlatFileUtil.Normal_RECON) &&
            ...
            ProcessBuilder pb =
                new ProcessBuilder(customScriptFile.getAbsolutePath());
            Process process;
            try {
                process = pb.start();
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    while (sc.hasNext()) {
                        ...
                        if (token.startsWith("lastName")) {
                            ConnectorObjectBuilder cObjBuilder =
                                new ConnectorObjectBuilder();
                            String[] tokenArray = token.split(":");
                            cObjBuilder.addAttribute("lastName",
                                                     tokenArray[1]);
        ...
        //then need to get data from target based on 'LatestToken' in other words incremental recon
            ...
            if (optionsList.contains(FlatFileUtil.LatestToken)) {
                ...
                searchResult =
                        getIncrementalSearchResult(reader, targetFile, uniqueAttribute,
                                                   query);

            } else {
                System.out.println("Performing full recon");
                searchResult =
                        getSearchResult(reader, targetFile, uniqueAttribute,
                                        query);
            }
            ...
            for (String accountID : accountIds) {
                ...
                for (String attrName : attrNames) {
                    System.out.println("Adding attr " + attrName +
                                       " with value " +
                                       attrNameValues.get(attrName) +
                                       " for accountID " + accountID);
                    cObjBuilder.addAttribute(attrName,
                                             attrNameValues.get(attrName));
                }
                cObjBuilder.addAttribute(FlatFileUtil.Incremental_Recon_Attribute,
                                         new Date().getTime());
                cObjBuilder.addAttribute(FlatFileUtil.LatestToken,
                                         new Date().getTime());
    ...
    private Map<String, HashMap<String, List<String>>> getIncrementalSearchResult(BufferedReader reader,
                                                                                  File targetFile,
    ...
    private Map<String, HashMap<String, List<String>>> getSearchResult(BufferedReader reader,
                                                                       File targetFile,
                                                                       String uniqueAttribute,
                                                                       Map<String, String> queryMap) {
        ...
        Map<String, HashMap<String, List<String>>> searchResult =
        ...
        try {
            ...
            throw new RuntimeException("Exception while reading the target file " +
                                       e.getMessage());
        }
    ...
    public Schema schema() {
        ...
        attrInfoSet.add(AttributeInfoBuilder.build(FlatFileUtil.Password,
                                                   String.class));
        attrInfoSet.add(AttributeInfoBuilder.build(FlatFileUtil.Incremental_Recon_Attribute,
                                                   String.class));
